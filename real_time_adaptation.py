...
class AdaptiveCBFParameterSelector:
    def __init__(self, model_name, scaler_name, distance_margin=0.001, step_size=0.02, epistemic_threshold=0.2):
...
def single_agent_simulation_traj(distance, velocity, theta, gamma1, gamma2, known_obs, controller, max_sim_time=30, adapt_cbf=False, scen = 1):
    dt = 0.05
...
if __name__ == "__main__":
    ...
    # gamma_history = np.array([[0.05, 0.05], [0.05, 0.05], [0.05, 0.05], [0.05, 0.05], [0.05, 0.15], [0.05, 0.15], [0.20, 0.20]])
    # plot_gamma_history(gamma_history)
    
    scenario_num = 2
    
    if scenario_num == 1:
        known_obs = np.array([[3.5, 0.65, 0.4], [3.5, 2.4, 0.5],
                                [6.5, 2.5, 1.1],
                                [8, 0.6, 0.4], [8, 2.2, 0.5],])
        
        trajectory_3, gamma_history = single_agent_simulation_traj(3.0, 0.5, 0.01, 0.05, 0.05, known_obs = known_obs, controller = 'mpc_cbf', adapt_cbf=True, scen = scenario_num)
        trajectory_opt2, _ = single_agent_simulation_traj(3.0, 0.5, 0.01, 0.1, 0.1, known_obs = known_obs, controller = 'optimal_decay_mpc_cbf', adapt_cbf=False, scen = scenario_num)
        trajectory_2, _ = single_agent_simulation_traj(3.0, 0.5, 0.01, 0.2, 0.2, known_obs = known_obs, controller = 'mpc_cbf', adapt_cbf=False, scen = scenario_num)
        trajectory_opt1, _ = single_agent_simulation_traj(3.0, 0.5, 0.01, 2.5, 2.5, known_obs = known_obs, controller = 'optimal_decay_cbf_qp', adapt_cbf=False, scen = scenario_num, max_sim_time=10)
        trajectory_1, _ = single_agent_simulation_traj(3.0, 0.5, 0.01, 0.03, 0.03, known_obs = known_obs, controller = 'mpc_cbf', adapt_cbf=False, scen = scenario_num)        
        
    elif scenario_num == 2:    
        # known_obs = np.array([[3, 0.65, 0.2], [3, 1.95, 0.2], [3, 3.25, 0.2], [3, 4.55, 0.2], [3, 5.85, 0.2],
        #                    [5.8, 0.9, 0.2], [5.8, 2.2, 0.2], [5.8, 3.5, 0.2], [5.8, 4.8, 0.2], [5.8, 6.1, 0.2],
        #                    [8.6, 1.15, 0.2], [8.6, 2.45, 0.2], [8.6, 3.75, 0.2], [8.6, 5.05, 0.2], [8.6, 6.35, 0.2],
        #                    [11.4, 1.4, 0.2], [11.4, 2.7, 0.2], [11.4, 4.0, 0.2], [11.4, 5.3, 0.2], [11.4, 6.6, 0.2],
        #                    [14.2, 0.65, 0.2], [14.2, 1.95, 0.2], [14.2, 3.25, 0.2], [14.2, 4.55, 0.2], [14.2, 5.85, 0.2],])
        ...
        # known_obs = np.array([[3, 1.95, 0.2], [3, 3.25, 0.2], [3, 4.55, 0.2],
        #                    [5.8, 2.2, 0.2], [5.8, 3.5, 0.2], [5.8, 4.8, 0.2], 
        #                    [8.6, 2.45, 0.2], [8.6, 3.75, 0.2], [8.6, 5.05, 0.2], 
        #                    [11.4, 2.7, 0.2], [11.4, 4.0, 0.2], [11.4, 5.3, 0.2], 
        #                    [14.2, 1.95, 0.2], [14.2, 3.25, 0.2], [14.2, 4.55, 0.2], ])
        
        trajectory_1, _ = single_agent_simulation_traj(3.0, 0.5, 0.01, 0.05, 0.05, known_obs = known_obs, controller = 'mpc_cbf', adapt_cbf=False, scen = scenario_num)
        trajectory_2, _ = single_agent_simulation_traj(3.0, 0.5, 0.01, 0.2, 0.2, known_obs = known_obs, controller = 'mpc_cbf', adapt_cbf=False, scen = scenario_num)
        trajectory_opt1, _ = single_agent_simulation_traj(3.0, 0.5, 0.01, 2.5, 2.5, known_obs = known_obs, controller = 'optimal_decay_cbf_qp', adapt_cbf=False, scen = scenario_num, max_sim_time=10)
        trajectory_opt2, _ = single_agent_simulation_traj(3.0, 0.5, 0.01, 0.19, 0.19, known_obs = known_obs, controller = 'optimal_decay_mpc_cbf', adapt_cbf=False, scen = scenario_num)
        trajectory_3, gamma_history = single_agent_simulation_traj(3.0, 0.5, 0.01, 0.05, 0.05, known_obs = known_obs, controller = 'mpc_cbf', adapt_cbf=True, scen = scenario_num)
